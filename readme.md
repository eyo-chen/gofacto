...
// Create a factory for Customer
customerFactory := gofacto.New(Customer{}).
                           WithDB(mysqlf.NewConfig(db)) // Assuming a MySQL connection (db)
...
## Insert
Use `Insert` to create and insert values into the database
```go
...
When building a list of values, `Overwrite` and `Overwrites` methods can be used to overwrite the fields.
```go
...
## WithTrait
When initializing the factory, use `WithTrait` method to set the trait functions. Then use `WithTrait` method to apply the trait functions when building the struct.
```go
...
customer, err := factory.Build(ctx).WithTrait("female").Insert()
// customer.Gender = Female
```


When building a list of values, `WithTrait` is used to apply one trait function to all the list of values, and `WithTraits` is used to apply multiple trait functions to the list of values.
```go
func setFemale(c *Customer) {
  c.Gender = Female
}
...
customers, err := factory.BuildList(ctx, 2).WithTrait("female").Insert()
// customers[0].Gender = Female
// customers[1].Gender = Female

customers, err := factory.BuildList(ctx, 2).WithTraits("female", "male").Insert()
// customers[0].Gender = Female
...
`gofacto:"struct:<structName>,table:<TableName>,foreignField:<ForeignField>"`<br>
- `sturctName` is the name of the associated struct. It is required.<br>
- `TableName` is the name of the table. It is optional, the snake case of the struct name(s) will be used if not provided.<br>
- `ForeignField` is the name of the foreign value fields within the struct. It is optional, and only required when using gorm.<br>
...
// build an order with one customer and overwrite the customer
c1 := Customer{}
...
```
The storage name will be used when inserting the struct into the database. <br>
...
It is optional, the snake case of the struct name will be used if not provided.<br>
...
type Order struct {
  ID          int
  User        Customer   `gorm:"foreignKey:CustomerID"`
  CustomerID  int        `gofacto:"struct:Customer,foreignField:User"`
  OrderDate   time.Time
  Amount      float64
}
```
In this case, `CustomerID` is the foreign key in `Order` struct, and `User` is the associated struct. `gofacto` tag is used to specify the associated struct and the foreign key field.

If the field name is as the same as the struct name, `foreignField` is optional.
```go
type Order struct {
  ID          int
...
3. The gofacto only supports basic associations relationship.
